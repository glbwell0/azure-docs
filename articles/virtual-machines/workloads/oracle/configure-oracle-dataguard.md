...
### Add required Azure CLI extension
...
With the default variables, the following creates a resource group named `Oracle-Lab` in the `eastus` location:
...
### Create virtual machines for the Oracle primary and standby databases

Create VM's by using the [az vm create](/cli/azure/vm) command. 

The following example creates two VMs named `OracleVM1` and `OracleVM2`. It also creates SSH keys, if they do not already exist in a default key location. To use a specific set of keys, use the `--ssh-key-value` option.
...
Change the <b>oracle</b> user password to: <b>OracleLab123</b>  <i>(enter again to verify)</i>
Switch to the Oracle superuser:

### Create the database on OracleVM1 
```bash 
sudo su - oracle
...
### Set up service on OracleVM1 (primary)
     ...
     Edit or create the <b>tnsnames.ora</b> file, which is in the $ORACLE_HOME/network/admin folder.
...
Edit or create the <b>listener.ora<b> file, which is in the $ORACLE_HOME/network/admin folder.
...
At the top of the screen, click <b>Connect</b> and select Bastion.

<image>
         
Enter the Username and Password and click the <b>Connect</b> button.
...
```bash
orapwd file=/u01/app/oracle/product/19.0.0/dbhome_1/dbs/orapwcdb1 password=OracleLab123 entries=10 format=12   
```
...
Finished recover at 29-JUN-21
Finished Duplicate Db at 29-JUN-21
```
...
Update or create the <b>tnsnames.ora</b> file on your client machine. This file is usually in $ORACLE_HOME/network/admin.
